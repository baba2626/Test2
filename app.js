...
async function updateCurrentPrizePool() {
  const prizePoolWei = await contract.methods.getPrizePool().call();
  const prizePoolEther = web3.utils.fromWei(prizePoolWei, "ether");
...
async function updateTimer() {
  ...
  const remainingTime = lastDrawTime + minimumDrawTime - currentTime;

  timer.innerText = new Date(remainingTime * 1000).toISOString().substr(11, 8);
}
